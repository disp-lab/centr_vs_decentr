rm(list=ls())
# 1/ LOAD DATA
sources_24h.name.main  <-c("OptDecentr"     , "NoRealloc" , "Auction"         , "P2P"         , "CNP"         , "Cluster")
sources_24h.name.file  <-c("MTSPc3swap_24h" , "TSP_24h"   , "AuctionSwap_24h" , "P2Pswap_24h" , "CNPswap_24h" , "ClusRaoSwap_24h")
                            MTSPc3swap_24h  <- TSP_24h   <-  AuctionSwap_24h  <- P2Pswap_24h  <- CNPswap_24h <-  ClusRaoSwap_24h <- list()
sources_24h         <-list( MTSPc3swap_24h  ,  TSP_24h    ,  AuctionSwap_24h  ,  P2Pswap_24h  ,  CNPswap_24h  ,  ClusRaoSwap_24h)
...
proba <- 0.5  #REPLACE BY THE DESIRED QUANTILE (proba<-0.5 or proba<-0.9)
